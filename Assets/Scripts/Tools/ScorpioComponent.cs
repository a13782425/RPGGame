...
public class ScorpioComponent : MonoBehaviour
{
    ...
    public void FindChild(ScriptTable objects)
    {
        ...
        while (itor.MoveNext())
        {
            ...
            Table.SetValue(itor.Current.Key,
                component == null ?
                m_Script.CreateObject(EngineUtils.FindChild(gameObject, info.GetValue("Path").ToString())) :
                m_Script.CreateObject(EngineUtils.FindChild(gameObject, info.GetValue("Path").ToString(), component)));
        }
    }
    public void OnStart()
    {
